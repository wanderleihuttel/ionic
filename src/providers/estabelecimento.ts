...
export class EstabelecimentoProvider {
  ...
  public listarProdutos(dados) {
      return this.http.post(this.path.url() + 'produtos', dados, this.options).map(res => res.json())
  }

  public produtosCategoria(dados) {
      return this.http.post(this.path.url() + 'produtos-categoria', dados, this.options).map(res => res.json())
  }
}
