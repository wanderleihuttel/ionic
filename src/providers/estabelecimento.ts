...
export class EstabelecimentoProvider {
  ...
  public listarProdutos(dados) {
      return this.http.get(this.path.url() + 'produtos?estabelecimento=' + dados.estabelecimento + '&limite=' + this.por_pagina + '&pular=' + dados.start, this.options).map(res => res.json())
  }
